...
Taking the first look at the data, the table is messy with rows that are not needed. The import and export data tables are separated at the moment.
...
We will therefore remove those irrelevant rows and promote the "Data Series" row as our table title. To consolidate the export and import tables, a column with the column name "trade" will be tagged to each row, whereby whether the row belongs to "Export" or "Import" will be categorised.
...
For this exercise, the period of January 2020 to December 2022 is to be studied. Therefore, only columns of 2020, 2021 and 2022 are identified and kept.
...
With that, tables of export and import data are merged together. Columns of trade data by months are gathered (pivoted) into one column. Noticed that some of data are in the unit of millions while some are in thousands. To align all data, we will exchange all data into dollars unit. 
```{r}
...
A table of trade_clean but in wide format is also produced and reserved for net export trade calculation.
...
A slopegraph of total trade volume with Singapore from each of continents is produced below for the period of 2020 to 2022 during the pandemic. The slopegraph is chosen as it is capable of revealing the trade volumes change between two time points. 
...
p1 <- trade_region %>% 
  ...
  newggslopegraph(year, Trade_Total, Market_,
                  Title = "Trade Volume with Singapore from Different Continents (in $billions)",
                  SubTitle = "From 2020 to 2022",
                  Caption = "Total trade value is the sum of export and import") +
...
p1
```
### Trade surplus/deficit from top 10 trading countries with Singapore
A positive balance of trade, known as a trade surplus, occurs when a country exports more goods than it imports. This usually means the country is earning more from its exports than it is spending on its imports, and it is generally seen as a sign of economic strength.
On the other hand, trade deficits occurs when a country imports more goods than it exports.  

Here, we will look at the top 10 trading volumes with Singapore in 2022 and their trade balance during the 3 years span. A horizontal plot is chosen here to enable friendly readability with sufficient horizontal space.
```{r}
...
p2 <- trade_clean_wide %>% 
  ...
  ggtitle('Net Export from 2022 Top 10 Total Trade Value Countries with Singapore') +
  labs(subtitle = "Net Export = Export - Import. Blue means Surplus, red means deficit.")

p2
```
### Top 10 trading countries volume monthly pattern
We are also interested in the trend of the trade volume by month from each of the top 10 countries, to see if there is any seasonality pattern. 
A cycle plot is captured to present how values vary over a period of time.
```{r}
#| fig-width: 12
#| label: fig-cycleplot
...
p3 <- ggplot(data = trade_cycle) +
  ...
  facet_wrap(~Month,
             ncol = 6) +
  theme_economist() +
    labs(title = "Top 10 trading countries trade volume by month",
         y = "Total Trade Amount ($millions)",
         x = NULL) +
    theme(plot.title = element_text(vjust = 2.5),
          legend.position = "none",
          axis.title.y = element_text(vjust = 2.5),
          panel.spacing.x = unit(0.85, "line"))
p3
```
## Insights and thoughts
i\. Singapore's bi-lateral trade with the rest of the world is in the process of recovering from the impact of COVID-19. Across the five continents, an upward trend is observed from 2020 to 2022, as seen in @fig-slopegraph

ii\. The biggest portion of Singapore's bi-lateral trade comes from Asia. According to @fig-cycleplot,
...
-   Most of Chinese New Year holidays fall in the month of February, both sides' work and operations could be in a slower pace as comapred to the rest of months.